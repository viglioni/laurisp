...
;;;###autoload
(defun new-laurisp-lib (lib-name)
  (interactive "sInsert lib name: ")
  (let* ((dir (join-path personal-lib-dir lib-name))
         (test-dir (join-path dir "test")))
    (make-directory dir)
    (make-directory test-dir)
    (let ((file-name (touch (concat lib-name ".el") dir))
          (readme (touch "README.org" dir))
          (eldev (touch "Eldev" dir))
          (test-file (touch (concat lib-name "-test.el") test-dir)))
      (echo-into file-name "laurisp-package")
      (echo-into readme (concat "* " lib-name))
      (echo-into eldev "(eldev-use-package-archive 'melpa)\n;(eldev-use-local-dependency \"\")")
      (echo-into test-file (format ";;; %s-test.el --- test suite -*- lexical-binding: t -*-" lib-name))
      (find-file file-name)
      (end-of-line)
      (yas-expand))))
